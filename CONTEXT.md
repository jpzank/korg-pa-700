...
Versioned, declarative mapping for one instrument model. It contains transport candidates, identity signatures, aliases, required setup, channels, exact presets, message templates, lifecycle state and evidence references. A profile describes data; it does not implement protocol state machines.

### Transport Identity

A manufacturer-independent description of USB MIDI variants and their expected
CoreMIDI source/destination names. Matching both endpoints can locate a
candidate, but only an explicit protocol identity response can confirm the
instrument. USB vendor/product IDs are retained as profile evidence and are not
queried through IOKit in v1.
...
### KORG Media Inventory

A deterministic, read-only metadata report for one `.SET` directory or one
recognized standalone resource. It stores relative paths, sizes,
classifications and warnings, never resource bytes or absolute paths. Inventory
recognition does not decode a proprietary format and cannot promote a MIDI
mapping.
