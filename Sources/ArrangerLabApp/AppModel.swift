...
struct BackdropDisplay: Identifiable, Equatable {
    let id: String
    let name: String
    let isMain: Bool

    var displayName: String { isMain ? "\(name) · principal" : name }
}

enum BackdropWindowMode: Equatable {
    case preview
    case fullscreen
}
...
final class AppModel: ObservableObject {
    enum Section: String, CaseIterable, Identifiable {
        ...
        var icon: String {
            ...
            case .batchMapping: return "square.stack.3d.up"
            case .guide: return "checklist"
            case .connection: return "cable.connector"
            case .korgMedia: return "externaldrive"
    ...
    @Published private(set) var backdropCues: [BackdropCue] = []
    @Published private(set) var backdropRenderState = BackdropRenderState()
    @Published private(set) var backdropStatus = "Nenhum visual ativo"
    @Published private(set) var backdropDisplays: [BackdropDisplay] = []
    @Published var selectedBackdropDisplayID: String?
    @Published private(set) var backdropWindowMode: BackdropWindowMode = .preview
    @Published private(set) var backdropWindowRequestID = UUID()
    ...
    private var backdropEventReducer = BackdropEventReducer()
    ...
    private let batchCatalogPersistenceQueue = DispatchQueue(
        label: "arrangerlab.catalog.persistence",
        qos: .utility
    )
    private var pendingBatchCatalogSaveTask: Task<Void, Never>?
    private var pendingBatchCatalogSaveID: UUID?
    private var catalogValidationTask: Task<Void, Never>?
    private var pendingShowPersistenceTask: Task<Void, Never>?
    private var showMutationTask: Task<Void, Never>?
    private var showMutationID: UUID?
    ...
    var selectedTransportMatch: TransportMatch? {
        guard let identity = profile.transportIdentity,
              let source = transport?.selectedSource,
              let destination = transport?.selectedDestination else { return nil }
        return TransportMatcher.matches(
            identity: identity,
            sources: [source],
            destinations: [destination]
        ).first
    }
    var selectedSourceMatchesProfile: Bool {
        guard let identity = profile.transportIdentity,
              let source = transport?.selectedSource else { return false }
        let normalized = TransportNameNormalizer.normalize(source.name)
        return identity.variants.contains { variant in
            variant.sourceAliases.contains {
                TransportNameNormalizer.normalize($0) == normalized
            }
        }
    }
    var selectedDestinationMatchesProfile: Bool {
        guard let identity = profile.transportIdentity,
              let destination = transport?.selectedDestination else { return false }
        let normalized = TransportNameNormalizer.normalize(destination.name)
        return identity.variants.contains { variant in
            variant.destinationAliases.contains {
                TransportNameNormalizer.normalize($0) == normalized
            }
        }
    }
    ...
    var identityVerified: Bool {
        pa700ConnectionHealth == .confirmed
            && transport?.isCurrentConnectionAuthorized == true
    }
    ...
    init() {
        ...
        do {
            let transport = try MIDITransport(requiresSessionAuthorization: true)
            ...
            handleTransportMatchResult(try autoConnectProfile(on: transport))
        ...
        restoreBackdropCues()
        refreshBackdropDisplays()
    }

    private func autoConnectProfile(on transport: MIDITransport) throws -> TransportMatchResult {
        guard let identity = profile.transportIdentity else { return .notFound }
        return try transport.autoConnect(matching: identity)
    }

    private func handleTransportMatchResult(
        _ result: TransportMatchResult,
        preserveReconnectIntent: Bool = false
    ) {
        switch result {
        case let .connected(match):
            connectionVerificationTask?.cancel()
            if !preserveReconnectIntent { manualReconnectInProgress = false }
            identityResult = "Não consultada"
            selectedSourceID = match.source.id
            selectedDestinationID = match.destination.id
            pa700ConnectionHealth = .candidate
            status = "\(match.variant.displayName) localizado como candidato; confirme a identidade"
        case .notFound:
            if !connected { pa700ConnectionHealth = .disconnected }
            status = "Nenhum par compatível com \(profile.model) foi encontrado; selecione manualmente"
        case let .ambiguous(matches):
            if !connected { pa700ConnectionHealth = .disconnected }
            status = "\(matches.count) pares compatíveis encontrados; escolha os endpoints manualmente"
        }
    }
        ...
        transport.onEndpointsChanged = { [weak self, weak transport] sources, destinations in
            DispatchQueue.main.async {
                ...
                if !self.connected {
                    ...
                    self.releaseBackdropForDisconnect()
                    self.clearActiveShowSelection()
                    self.showStatus = "PA700 desconectado. Reconecte antes de aplicar uma música."
                }
                guard !self.connected, let identity = self.profile.transportIdentity else { return }
                ...
                guard matches.count == 1, let match = matches.first else {
                    if matches.count > 1 {
                        self.status = "\(matches.count) pares compatíveis encontrados; escolha os endpoints manualmente"
                    }
                    return
                }
                do {
                    try transport.connect(sourceID: match.source.id, destinationID: match.destination.id)
                    self.selectedSourceID = match.source.id
                    self.selectedDestinationID = match.destination.id
                    self.pa700ConnectionHealth = .candidate
                    self.identityResult = "Não consultada"
                    self.status = "\(match.variant.displayName) localizado; confirme a identidade explicitamente"
                    self.showStatus = "PA700 reconectado; confirme a identidade antes de aplicar uma música."
                } catch {
                    self.pa700ConnectionHealth = .unresponsive
                    self.showStatus = "A reconexão MIDI falhou. Verifique o PA700 e tente novamente."
                    self.fail(error)
                }
            }
        }
    ...
    private func beginConnectionVerification(status message: String) throws {
        ...
        status = message
        try transport.sendUniversalIdentityRequest()
    ...
    func resetPA700Connection() {
        ...
        cancelPendingShowMutation()
        connectionVerificationTask?.cancel()
        invalidatePA700LiveState()
        releaseBackdropForDisconnect()
        ...
        do {
            let replacement = try MIDITransport(requiresSessionAuthorization: true)
            transport = replacement
            configureTransportCallbacks(replacement)
            sources = replacement.sources
            destinations = replacement.destinations
            let result = try autoConnectProfile(on: replacement)
            handleTransportMatchResult(result, preserveReconnectIntent: true)
            switch result {
            case .connected:
                break
    ...
    func connect() {
        do {
            ...
            invalidatePA700LiveState()
            releaseBackdropForDisconnect()
            showStatus = "Conexão MIDI alterada; confirme o PA700 antes de aplicar uma música."
            ...
            if connected {
                ...
                pa700ConnectionHealth = .candidate
                identityResult = "Não consultada"
                if let match = selectedTransportMatch {
                    status = "\(match.variant.displayName) conectado como candidato; confirme a identidade"
                } else {
                    status = "Endpoints conectados manualmente; identidade ainda não confirmada"
                }
            } else {
                pa700ConnectionHealth = .disconnected
                status = "Desconectado"
                showStatus = "PA700 desconectado. Reconecte antes de aplicar uma música."
            }
            objectWillChange.send()
        } catch {
            showStatus = "Não foi possível concluir a alteração da conexão MIDI."
            fail(error)
        }
    }
    ...
    func autoConnect() {
        do {
            guard let transport else { throw ArrangerLabError.endpointUnavailable }
            handleTransportMatchResult(try autoConnectProfile(on: transport))
        } catch { fail(error) }
    }
    ...
    func disconnect() {
        cancelPendingShowMutation()
        showStatus = "Desconectando do PA700…"
        auditionTask?.cancel()
        auditionTask = nil
        ...
        if let transport {
            do { try transport.connect(sourceID: nil, destinationID: nil) }
            catch {
                if let arrangerError = error as? ArrangerLabError,
                   case .panicDeliveryFailed = arrangerError {
                    panicError = error
                } else {
        ...
        let catalogSaved = flushBatchCatalogSilently()
        clearActiveShowSelection()
        invalidatePA700LiveState()
        releaseBackdropForDisconnect()
        pa700ConnectionHealth = connected ? .unresponsive : .disconnected
        expert.expire()
        var warnings: [String] = []
        if let panicError {
            warnings.append("Panic não confirmado")
            lastError = panicError.localizedDescription
        }
        if let disconnectError {
            warnings.append("desconexão MIDI não confirmada")
            lastError = disconnectError.localizedDescription
        }
        if !catalogSaved { warnings.append("catálogo ainda não salvo") }
        if connected {
            status = "Falha ao desconectar: \(warnings.joined(separator: " · "))"
            showStatus = "Desconexão não confirmada. Pare o PA700 pelo painel e tente novamente."
        } else {
            status = warnings.isEmpty
                ? "Desconectado"
                : "Desconectado; \(warnings.joined(separator: " · "))"
            showStatus = panicError == nil
                ? "PA700 desconectado. Reconecte antes de aplicar uma música."
                : "PA700 desconectado, mas o Panic não foi confirmado no instrumento."
        }
        objectWillChange.send()
    }
    ...
    func panic() {
        ...
        clearActiveShowSelection()
        invalidatePA700LiveState()
        backdropEventReducer.returnToAmbient(at: backdropTimestamp())
        backdropRenderState = backdropEventReducer.state
        backdropStatus = backdropRenderState.activePresetID == nil
            ? "Nenhum visual ativo"
            : "Panic · ambiente seguro"
        do {
            guard let transport else { throw ArrangerLabError.endpointUnavailable }
            try transport.panic()
            ...
            status = "Panic tentado, mas a entrega não foi confirmada"
            showStatus = "Panic não confirmado. Interrompa o PA700 pelo painel e reconecte."
        }
    }
    ...
    func stopBatchMapping() {
        ...
        let catalogSaved = flushBatchCatalogSilently()
        do {
            try sendPanicOrThrow()
            status = catalogSaved
                ? "Captura em lote parada · \(batchSoundEntries.count) timbres únicos salvos"
                : "Captura parada · catálogo ainda não salvo"
        } catch { fail(error) }
    }

    func createNewBatchMappingSession() {
        if isBatchMapping {
            do { try sendPanicOrThrow() }
            catch {
                fail(error)
                return
            }
        ...
        catalogValidationTask = Task { [weak self] in
            ...
            do {
                ...
                do {
                    ...
                    lastError = error.localizedDescription
                    status = "Varredura cancelada · Panic não confirmado; pare o PA700 pelo painel"
                }
            } catch {
                audioRecorder.stopSilently()
                isCatalogValidating = false
    ...
    func confirmFastCatalogValidation(heard: Bool) {
        ...
        do {
            ...
            let attachments: [UUID: URL] = catalogValidationAudioSourceURL.map {
                [evidence.id: $0]
            } ?? [:]
            let publishedURL = try ArrLabPackage.publish(
                .init(manifest: manifest, events: sweepEvents, analysis: analysis),
                to: url,
                audioAttachments: attachments
            )
            catalogValidationExperimentURL = publishedURL
    ...
    func verifyFullCatalogFromSampling() {
        ...
        do {
            ...
            let attachments = Dictionary(uniqueKeysWithValues: sweep.analysis.audioEvidence.map {
                ($0.id, sweepURL.appendingPathComponent($0.relativePath))
            })
            let publishedURL = try ArrLabPackage.publish(
                .init(manifest: manifest, events: sweep.events, analysis: analysis),
                to: url,
                audioAttachments: attachments
            )

            let promoted = collector.markAllVerifiedBySampling(experimentPath: publishedURL.path, now: now)
            batchCollector = collector
            syncBatchCatalogState(from: collector)
            try saveBatchCatalog()
            catalogValidationExperimentURL = publishedURL
    ...
    func queryIdentity() {
        do { try beginConnectionVerification(status: "Consulta de identidade enviada explicitamente…") }
        catch { fail(error) }
    }
    ...
    func unlockExpert(typedModel: String) {
        do {
            guard identityVerified else { throw ArrangerLabError.deviceIdentityRequired }
            try expert.unlock(typedModel: typedModel, connectedModel: profile.model)
            status = "Expert ativo até desconectar ou fechar"
        } catch { fail(error) }
    }
    ...
    func replayCurrent() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            let result = Result { try midi?.replay(replayEvents, speed: speed) }
            DispatchQueue.main.async {
                switch result {
                case .success:
                    self?.status = "Replay encerrado com Panic"
    ...
    func stopClock() {
        ...
        if let error = transport?.stopClock() {
            lastError = error.localizedDescription
            status = "Stop + Panic tentados, mas a entrega não foi confirmada"
        } else {
            status = "Stop + Panic enviados; restaure Clock Source = Internal"
        }
    }
    ...
    func stopGuidedArrangerClock() {
        let stopError: Error?
        if let transport {
            stopError = transport.stopClock()
        } else {
            stopError = ArrangerLabError.endpointUnavailable
        }
        arrangerClockRunning = false
        arrangerStopSent = stopError == nil
        if stopError != nil { arrangerStoppedConfirmed = false }
        do {
            let evidence = try audioRecorder.stop()
            arrangerAudioEvidence = evidence
            if let stopError {
                lastError = stopError.localizedDescription
                status = "Stop + Panic não confirmados; pare o PA700 pelo painel antes de continuar"
            } else {
                status = "Stop + Panic enviados; confirme que parou e restaure Clock Source = Internal"
            }
        } catch {
            if let stopError {
                lastError = "\(stopError.localizedDescription) · Áudio: \(error.localizedDescription)"
                status = "Stop + Panic não confirmados; o áudio também não pôde ser finalizado"
            } else {
    ...
    func saveArrangerTransportVerification() {
        ...
        do {
            ...
            let attachments: [UUID: URL] = arrangerAudioSourceURL.map {
                [evidence.id: $0]
            } ?? [:]
            let publishedURL = try ArrLabPackage.publish(
                .init(manifest: manifest, events: capturedEvents, analysis: analysis),
                to: url,
                audioAttachments: attachments
            )
            arrangerTransportVerified = true
            arrangerTransportExperimentURL = publishedURL
            status = "Verified: Arranger Start/Stop salvo em \(publishedURL.path)"
        } catch { fail(error) }
    }
    ...
    func saveExperiment() {
        ...
        do {
            ...
            var attachments: [UUID: URL] = [:]
            for evidence in audioEvidence {
                if let source = audioSourceURLs[evidence.id] {
                    attachments[evidence.id] = source
                }
            }
            let publishedURL = try ArrLabPackage.publish(
                .init(manifest: manifest, events: events, analysis: analysis),
                to: url,
                audioAttachments: attachments
            )
            lastSavedExperimentURL = publishedURL
            status = mappingStatus == .verified
                ? "Verified: partVolume salvo em \(publishedURL.path)"
                : "Experimento Draft salvo em \(publishedURL.path)"
        } catch { fail(error) }
    }
    ...
    func deleteShowPreset(_ preset: ShowPreset) {
        do {
            ...
            let updatedBackdropCues = backdropCues.filter { $0.presetID != preset.id }
            if updatedBackdropCues != backdropCues {
                try persistBackdropCues(updatedBackdropCues)
                backdropCues = updatedBackdropCues
            }
            if backdropRenderState.activePresetID == preset.id {
                backdropEventReducer.deactivate(at: backdropTimestamp())
                backdropRenderState = backdropEventReducer.state
                backdropStatus = "Nenhum visual ativo"
            }
            if activeShowPresetID == preset.id { clearActiveShowSelection() }
            if pendingShowConfirmationID == preset.id { pendingShowConfirmationID = nil }
    ...
    func testShowPreset(_ preset: ShowPreset) -> Bool {
        guard identityVerified else {
            showStatus = connected
                ? "Confirme a identidade desta conexão antes de testar a música"
                : "Conecte e confirme o PA700 para testar esta música"
            return false
        }
        do {
            ...
            if preset.hasDirectSetup {
                ...
                completedStatus = "\(preset.songTitle) preparada diretamente no PA700"
                confirmationID = nil
                ...
                completedStatus = "SongBook \(songBookNumber) enviado. Confira o PA700 e confirme a música."
                confirmationID = preset.id
            } else {
            ...
            startShowMutation(
                schedule: schedule,
                progressStatus: "Testando \(preset.songTitle)…",
                failureStatus: "Falha ao testar \(preset.songTitle)"
            ) { [weak self] in
                self?.pendingShowConfirmationID = confirmationID
                self?.showStatus = completedStatus
            }
            return true
        } catch {
    ...
    func applyShowPreset(_ preset: ShowPreset, setListItemID: UUID? = nil) -> Bool {
        ...
        do {
            ...
            startShowMutation(
                schedule: schedule,
                progressStatus: "Aplicando \(preset.songTitle)…",
                failureStatus: "Falha ao aplicar \(preset.songTitle); a música ativa não mudou"
            ) { [weak self] in
                guard let self else { return }
                ...
                if preset.hasDirectSetup {
                    let styleName = preset.arrangerStyleID.flatMap { id in
                        self.arrangerStyles.first { $0.id == id }?.displayName
                    } ?? "Style"
    ...
    private func startShowMutation(
        schedule: [ScheduledMIDIMessage],
        ...
        guard let transport else {
            ...
            fail(ArrangerLabError.endpointUnavailable)
            return
        }
        cancelPendingShowMutation()
        let mutationID = UUID()
        showMutationID = mutationID
        showStatus = progressStatus
        status = progressStatus
        showMutationTask = Task { @MainActor [weak self, transport] in
            do {
                try await transport.sendScheduledAsync(schedule)
                guard let self,
                      !Task.isCancelled,
                      self.showMutationID == mutationID,
                      self.transport === transport else { return }
                guard transport.isCurrentConnectionAuthorized else {
                    self.showStatus = "A conexão mudou durante o envio; aplique novamente a música."
                    self.showMutationID = nil
                    self.showMutationTask = nil
                    return
                }
                // All output callbacks have been enqueued synchronously by the
                // transport. Process them before publishing the new commanded
                // snapshot so generic device-mutation invalidation cannot stale it.
                self.flushPendingMIDIEvents()
                onSuccess()
                self.showMutationID = nil
                self.showMutationTask = nil
            } catch is CancellationError {
                guard let self, self.showMutationID == mutationID else { return }
                self.showStatus = "Envio cancelado; selecione novamente a música"
                self.showMutationID = nil
                self.showMutationTask = nil
            } catch {
                guard let self, self.showMutationID == mutationID else { return }
                self.showStatus = failureStatus
                self.showMutationID = nil
                self.showMutationTask = nil
                self.fail(error)
            }
    ...
    private func cancelPendingShowMutation() {
        transport?.cancelScheduledSends()
        showMutationTask?.cancel()
        showMutationTask = nil
        showMutationID = nil
        pendingShowConfirmationID = nil
    }
    ...
    func backdropCue(for presetID: UUID) -> BackdropCue {
        backdropCues.first(where: { $0.presetID == presetID })
            ?? BackdropCue(presetID: presetID)
    }

    func isBackdropActive(for presetID: UUID) -> Bool {
        backdropRenderState.activePresetID == presetID && !backdropRenderState.isBlackout
    }

    @discardableResult
    func saveBackdropCue(_ candidate: BackdropCue) -> Bool {
        do {
            var cue = candidate
            cue.updatedAt = Date()
            try cue.validate()
            var updated = backdropCues
            if let index = updated.firstIndex(where: { $0.presetID == cue.presetID }) {
                updated[index] = cue
            } else {
                updated.append(cue)
            }
            try persistBackdropCues(updated)
            backdropCues = updated
            if backdropRenderState.activePresetID == cue.presetID {
                backdropEventReducer.updateActiveCue(cue, at: backdropTimestamp())
                backdropRenderState = backdropEventReducer.state
                backdropStatus = cue.isEnabled
                    ? "Visual atualizado para \(showPresets.first(where: { $0.id == cue.presetID })?.songTitle ?? "a música ativa")"
                    : "Backdrop desativado para a música ativa"
            }
            return true
        } catch {
            backdropStatus = "Não foi possível salvar o visual"
            fail(error)
            return false
        }
    }
    ...
    @discardableResult
    func activateBackdrop(for preset: ShowPreset) -> Bool {
        let cue = backdropCue(for: preset.id)
        guard cue.isEnabled else {
            backdropStatus = "Backdrop desativado em \(preset.songTitle)"
            return false
        }
        backdropEventReducer.activate(cue, at: backdropTimestamp())
        backdropRenderState = backdropEventReducer.state
        backdropStatus = "\(preset.songTitle) · \(cue.palette.displayName) · visual ao vivo"
        return true
    }

    func blackoutBackdrop() {
        backdropEventReducer.blackout(at: backdropTimestamp())
        backdropRenderState = backdropEventReducer.state
        backdropStatus = "Blackout ativo"
    }

    func returnBackdropToAmbient() {
        backdropEventReducer.returnToAmbient(at: backdropTimestamp())
        backdropRenderState = backdropEventReducer.state
        if let presetID = backdropRenderState.activePresetID,
           let preset = showPresets.first(where: { $0.id == presetID }) {
            backdropStatus = "\(preset.songTitle) · ambiente ativo"
        } else {
            backdropStatus = "Nenhum visual ativo"
        }
    }

    func previewBackdropPaint() {
        guard let activePresetID = backdropRenderState.activePresetID,
              backdropRenderState.cue?.isEnabled == true,
              !backdropRenderState.isBlackout else {
            backdropStatus = "Ative o visual de uma música antes do teste"
            return
        }
        let startedAt = backdropTimestamp()
        for (index, note) in [60, 64, 67, 72].enumerated() {
            let message = MIDIMessage.noteOn(
                channel: UInt8(index == 3 ? 1 : 0),
                note: UInt8(note),
                velocity: UInt8(72 + index * 14)
            )
            _ = backdropEventReducer.consume(
                MIDIEvent(
                    timestampNanoseconds: startedAt + UInt64(index) * 24_000_000,
                    direction: .input,
                    endpointUniqueID: 0,
                    endpointName: "Preview local do backdrop",
        ...
        backdropRenderState = backdropEventReducer.state
        backdropStatus = "Teste visual local · nenhum MIDI enviado"

        Task { @MainActor [weak self] in
            try? await Task.sleep(nanoseconds: 1_200_000_000)
            guard let self,
                  self.backdropRenderState.activePresetID == activePresetID else { return }
            let releasedAt = self.backdropTimestamp()
            for (index, note) in [60, 64, 67, 72].enumerated() {
                ...
                _ = self.backdropEventReducer.consume(
                    MIDIEvent(
                        timestampNanoseconds: releasedAt + UInt64(index),
                        direction: .input,
                        endpointUniqueID: 0,
                        endpointName: "Preview local do backdrop",
                        rawBytes: message.canonicalBytes,
            ...
            self.backdropRenderState = self.backdropEventReducer.state
        }
    }

    func presentBackdrop(_ mode: BackdropWindowMode) {
        backdropWindowMode = mode
        backdropWindowRequestID = UUID()
    }

    func selectBackdropDisplay(_ id: String) {
        guard backdropDisplays.contains(where: { $0.id == id }) else { return }
        selectedBackdropDisplayID = id
        UserDefaults.standard.set(id, forKey: "arrangerlab.backdropDisplayID")
        if backdropWindowMode == .fullscreen {
            backdropWindowRequestID = UUID()
        }
    }

    func refreshBackdropDisplays() {
        let screens = NSScreen.screens
        backdropDisplays = screens.compactMap { screen in
        ...
        let stored = selectedBackdropDisplayID
            ?? UserDefaults.standard.string(forKey: "arrangerlab.backdropDisplayID")
        if let stored, backdropDisplays.contains(where: { $0.id == stored }) {
            selectedBackdropDisplayID = stored
        } else {
            selectedBackdropDisplayID = backdropDisplays.first(where: { !$0.isMain })?.id
                ?? backdropDisplays.first?.id
        }
        if let selectedBackdropDisplayID {
            UserDefaults.standard.set(selectedBackdropDisplayID, forKey: "arrangerlab.backdropDisplayID")
        }
        backdropWindowRequestID = UUID()
    }

    func backdropScreen(for id: String?) -> NSScreen? {
        guard let id else { return NSScreen.main }
        return NSScreen.screens.first { screen in
            (screen.deviceDescription[NSDeviceDescriptionKey("NSScreenNumber")] as? NSNumber)?.stringValue == id
        } ?? NSScreen.main
    }
    ...
    func auditionCatalogSound(id: String, target: KeyboardPartTarget, partName: String) {
        ...
        auditionTask = Task { [weak self] in
            ...
            do {
                ...
                do {
                    try sendPanicOrThrow()
                    auditionMessage = "Audição cancelada · Panic enviado"
                } catch {
                    lastError = error.localizedDescription
                    auditionMessage = "Audição cancelada · Panic não confirmado; pare o PA700 pelo painel"
                }
            } catch {
                audioRecorder.stopSilently()
                auditioningSoundID = nil
    ...
    func confirmCatalogAudition(matches: Bool) {
        ...
        do {
            ...
            let publishedURL = try ArrLabPackage.publish(
                .init(manifest: manifest, events: capturedEvents, analysis: analysis),
                to: url,
                audioAttachments: [evidence.id: source]
            )
            if var collector = batchCollector {
                collector.markVerified(id: id, experimentPath: publishedURL.path)
                batchCollector = collector
                syncBatchCatalogState(from: collector)
                try saveBatchCatalog()
            }
            auditionExperimentURL = publishedURL
            auditionAudioEvidence = nil
            auditionAudioSourceURL = nil
    ...
    func saveSongBookVerification() {
        ...
        do {
            ...
            songBookExperimentURL = publishedURL
            status = "Verified: SongBook 9000 salvo em \(publishedURL.path)"
        } catch { fail(error) }
    }
    ...
    func confirmExpressionAndSave() {
        ...
        do {
            ...
            partExpressionExperimentURL = publishedURL
    ...
    func confirmPanAndSave() {
        ...
        do {
            ...
            partPanExperimentURL = publishedURL
    ...
    func confirmDamperAndSave() {
        ...
        do {
            ...
            partDamperExperimentURL = publishedURL
    ...
    private func savePresetExperiment() {
        do {
            ...
            var attachments: [UUID: URL] = [:]
            for evidence in capturedAudio {
                if let source = presetAudioSourceURLs[evidence.id] {
                    attachments[evidence.id] = source
                }
            }
            let publishedURL = try ArrLabPackage.publish(
            ...
            presetExperimentURL = publishedURL
            persistedPresetSummary = notes.first ?? ""
            status = mappingStatus == .verified
                ? "Verified: preset exato A-B-A salvo em \(publishedURL.path)"
    ...
    private func receive(_ batch: [MIDIEvent]) {
        ...
        let batch = batch.filter(transport.isEventFromCurrentSession)
        ...
        if batch.contains(where: { $0.direction == .output }) {
            // Any successful outbound mutation may have changed the physical
            // keyboard. Show application flushes these events before publishing
            // its replacement commanded snapshot.
            markCommandedPA700StateStale()
            markObservedPA700StateStale()
        }
        ...
        for event in batch {
            ...
            if backdropEventReducer.consume(event) { backdropChanged = true }
            ...
            if driver.identifies(message) {
                do {
                    try transport.authorizeCurrentConnection(from: event)
                } catch {
                    // A reconnect can invalidate an already-batched identity
                    // reply. Ignore that stale reply without poisoning or
                    // panicking the replacement candidate session.
                    continue
                }
        ...
        if backdropChanged { backdropRenderState = backdropEventReducer.state }
    }
    ...
    private func releaseBackdropForDisconnect() {
        backdropEventReducer.disconnect(at: backdropTimestamp())
        backdropRenderState = backdropEventReducer.state
        if let presetID = backdropRenderState.activePresetID,
           let preset = showPresets.first(where: { $0.id == presetID }) {
            backdropStatus = "\(preset.songTitle) · sem MIDI, ambiente ativo"
        }
    }
    ...
    private func saveBatchCatalog() throws {
        pendingBatchCatalogSaveTask?.cancel()
        pendingBatchCatalogSaveTask = nil
        pendingBatchCatalogSaveID = nil
        guard let catalog = batchCollector?.catalog, let url = batchCatalogURL else { return }
        try batchCatalogPersistenceQueue.sync {
    ...
    private func saveBatchCatalogSilently() {
        ...
        pendingBatchCatalogSaveTask?.cancel()
        let saveID = UUID()
        pendingBatchCatalogSaveID = saveID
        let queue = batchCatalogPersistenceQueue
        pendingBatchCatalogSaveTask = Task { @MainActor [weak self] in
            do {
                try await Task.sleep(nanoseconds: 500_000_000)
                try Task.checkCancellation()
                ...
                guard let self, self.pendingBatchCatalogSaveID == saveID else { return }
                self.pendingBatchCatalogSaveTask = nil
                self.pendingBatchCatalogSaveID = nil
                if case let .failure(error) = result {
                    self.lastError = "Catálogo não salvo: \(error.localizedDescription)"
                    self.status = "Alterações do catálogo ainda não foram salvas"
                }
            } catch is CancellationError {
                // A newer immutable snapshot superseded this pending save.
            } catch {
                guard let self, self.pendingBatchCatalogSaveID == saveID else { return }
                self.pendingBatchCatalogSaveTask = nil
                self.pendingBatchCatalogSaveID = nil
    ...
    @discardableResult
    private func flushBatchCatalogSilently() -> Bool {
        do {
            try saveBatchCatalog()
            return true
        } catch {
            lastError = "Catálogo não salvo: \(error.localizedDescription)"
            status = "Alterações do catálogo ainda não foram salvas"
            return false
        }
    }
    ...
    private func backdropCueURL() throws -> URL {
        try showDirectory().appendingPathComponent("backdrop-cues.json")
    }
    ...
    private func persistBackdropCues(_ cues: [BackdropCue]) throws {
        try BackdropCueStore.save(cues, to: backdropCueURL())
    }
    ...
    private func restoreShowPerformance() {
        do {
            ...
            activeShowSetListID = storedSetListID ?? showSetLists.first?.id
            if let activeShowSetListID {
                UserDefaults.standard.set(activeShowSetListID.uuidString, forKey: "arrangerlab.activeShowSetListID")
            }
        } catch {
    ...
    private func restoreBackdropCues() {
        do {
            backdropCues = try BackdropCueStore.load(from: backdropCueURL())
        } catch {
            lastError = "Visuais do backdrop não puderam ser carregados: \(error.localizedDescription)"
        }
    }

    private func backdropTimestamp() -> UInt64 {
    ...
    private func restoreLatestVerification() {
        do {
            ...
            for url in urls {
                guard let experiment = try? ArrLabPackage.loadPublished(from: url) else { continue }
                ...
                if manifest.mappingID == "partVolume", !restoredVolume {
                    ...
                    previousOutputConfirmed = analysis.manualConfirmations.contains {
                        $0.confirmed && $0.prompt == "Mac to PA700 C4 output was audible"
                    }
                    previousPresetConfirmed = analysis.manualConfirmations.contains {
                        $0.confirmed && $0.prompt == "ArrangerLab MIDI preset configured"
                    }
                    let audio = analysis.audioEvidence
                    if audio.count >= 4 {
    ...
    private func sendPanicOrThrow() throws {
        guard let transport else { throw ArrangerLabError.endpointUnavailable }
        try transport.panic()
    }
    private func fail(_ error: Error) {
        ...
        do {
            try transport.panic()
            status = "Falha; Panic preventivo enviado"
        } catch {
            lastError = "\(originalMessage) · Panic: \(error.localizedDescription)"
            status = "Falha; Panic preventivo não confirmado"
        }
    }
