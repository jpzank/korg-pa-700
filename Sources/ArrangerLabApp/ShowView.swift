...
struct ShowView: View {
    ...
    @State private var editorBackdropCue = BackdropCue(presetID: UUID())
    @State private var editorBackdropOriginal = BackdropCue(presetID: UUID())
        ...
        Group {
            if let preset {
                ShowChartReader(
                    ...
                    backdropCue: model.backdropCue(for: preset.id),
                    isBackdropActive: model.isBackdropActive(for: preset.id),
                    isConnected: model.connected && model.pa700ConnectionHealth == .confirmed,
                    focusMode: focusMode,
                    ...
                    onActivateBackdrop: { model.activateBackdrop(for: preset) }
                )
            } else {
                VStack(spacing: 14) {
    ...
    private var emptyState: some View {
        VStack(spacing: 14) {
            ...
            Text("Prepare seu repertório")
                .font(.title2.weight(.semibold))
            Text("Crie uma música ou importe PDFs de cifras que você tem direito de usar.")
                .foregroundStyle(.secondary)
    ...
    private var editorIsDirty: Bool {
        ...
        return chartChanged || normalized != editorOriginal || editorBackdropCue != editorBackdropOriginal
    }
    ...
    private func completePendingWorkspaceAction(saving: Bool) {
        ...
        if saving {
            ...
            editorBackdropCue = editorBackdropOriginal
    ...
    private func performWorkspaceAction(_ action: PendingShowWorkspaceAction) {
        ...
        case let .editMusic(id, returnToLibrary):
            ...
            editorBackdropCue = model.backdropCue(for: preset.id)
            editorBackdropOriginal = editorBackdropCue
        ...
        case let .newMusic(returnToLibrary):
            ...
            editorDraft = preset
            editorOriginal = preset
            editorBackdropCue = BackdropCue(presetID: preset.id)
            editorBackdropOriginal = editorBackdropCue
            editorChartText = ""
            editorPresented = true
...
private struct ShowChartReader: View {
    ...
    private var backdropButton: some View {
        let title = isBackdropActive ? "Visual ao vivo" : "Ativar visual"
        let icon = isBackdropActive ? "sparkles.rectangle.stack.fill" : "sparkles.rectangle.stack"
        let button = Button(title, systemImage: icon, action: onActivateBackdrop)
            .disabled(!backdropCue.isEnabled || isEditingContent)
            .help(backdropHelp)
            .accessibilityValue(isBackdropActive ? "Ativo no backdrop" : "Não projetado")
        if isBackdropActive {
            button
                .buttonStyle(.bordered)
    ...
    private var backdropHelp: String {
        if !backdropCue.isEnabled { return "Ative o backdrop em Editar música" }
        if isEditingContent { return "Salve ou conclua a edição antes de mudar o visual ao vivo" }
        if isBackdropActive { return "Reiniciar o visual de \(preset.songTitle)" }
        return "Projetar o visual desta música sem enviar MIDI ao PA700"
    }
...
private final class ShowContinuousScrollDriver: ObservableObject {
    ...
    private func tick() {
        ...
        guard let scrollView, let documentView = scrollView.documentView else {
            // SwiftUI can replace the native scroll view while focus chrome is
            // shown or hidden. Keep the user's play state so attach(to:) can
            // resume on the replacement instead of leaving a paused driver
            // behind an active pause button.
            targetY = nil
            suspendTimer()
            return
        }
        ...
        if let targetY, abs(currentY - targetY) > 2 {
            // Respect a deliberate manual scroll while auto-scroll is active.
            self.targetY = currentY
        } else if targetY == nil {
            targetY = currentY
        }
        let nextY = min(maximumY, (targetY ?? currentY) + pointsPerSecond * elapsed)
        targetY = nextY
...
private struct ShowConnectionStrip: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Button {
                if model.pa700ConnectionHealth == .candidate {
                    model.queryIdentity()
                } else {
    ...
    private var backdropMenu: some View {
        Menu {
            Button("Abrir preview", systemImage: "macwindow") {
                model.presentBackdrop(.preview)
                openWindow(id: "backdrop")
            }
            Button("Tela cheia no monitor selecionado", systemImage: "rectangle.inset.filled") {
                model.presentBackdrop(.fullscreen)
                openWindow(id: "backdrop")
            }
            Button("Testar pintura", systemImage: "paintbrush.pointed") {
                model.previewBackdropPaint()
            }
            .disabled(
                model.backdropRenderState.activePresetID == nil
                    || model.backdropRenderState.isBlackout
            )
            Menu("Monitor") {
                ForEach(model.backdropDisplays) { display in
                    Button {
                        model.selectBackdropDisplay(display.id)
                    } label: {
                        if model.selectedBackdropDisplayID == display.id {
            ...
            Divider()
            if model.backdropRenderState.isBlackout {
                Button("Voltar ao ambiente", systemImage: "sun.haze") {
                    model.returnBackdropToAmbient()
                }
            } else {
                Button("Blackout", systemImage: "moon.fill", role: .destructive) {
                    model.blackoutBackdrop()
                }
            }
        } label: {
            Label(
                model.backdropRenderState.isBlackout ? "Blackout" : "Backdrop",
                systemImage: model.backdropRenderState.isBlackout
                    ? "rectangle.fill"
                    : "sparkles.rectangle.stack"
            )
        }
        .help("\(model.backdropStatus) · \(selectedBackdropDisplayName)")
        .accessibilityLabel("Controles do backdrop")
        .accessibilityValue(model.backdropStatus)
        .fixedSize()
    }

    private var selectedBackdropDisplayName: String {
        model.backdropDisplays.first(where: { $0.id == model.selectedBackdropDisplayID })?.displayName
            ?? "monitor não selecionado"
    }

    private var connectionIndicatorIcon: String {
        switch model.pa700ConnectionHealth {
        case .confirmed: "circle.fill"
        case .candidate: "questionmark.circle.fill"
    ...
    private var connectionIndicatorColor: Color {
        ...
        case .candidate, .reconnecting: LabTheme.draft
    ...
    private var connectionButtonLabel: String {
        ...
        case .candidate: "Confirmar identidade do PA700"
        case .reconnecting: "Confirmando identidade do PA700"
    ...
    private var connectionTitle: String {
        ...
        case .candidate: "PA700 localizado via USB"
        case .reconnecting: "Confirmando o PA700"
    ...
    private var connectionSubtitle: String {
        ...
        case .candidate: "Clique no indicador para enviar a consulta universal de identidade"
        case .reconnecting: "Aguardando a resposta universal de identidade…"
