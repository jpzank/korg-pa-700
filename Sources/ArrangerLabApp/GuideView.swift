...
struct GuideView: View {
    ...
    private func complete(_ candidate: GuideStep) -> Bool {
        switch candidate {
        case .connection: return model.connected && model.selectedTransportMatch != nil
    ...
    @ViewBuilder private var stepContent: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            case .connection:
                ...
                CheckLine(
                    ok: model.selectedSourceMatchesProfile,
                    text: "Entrada reconhecida: \(model.transport?.selectedSource?.name ?? "nenhuma")"
                )
                CheckLine(
                    ok: model.selectedDestinationMatchesProfile,
                    text: "Saída reconhecida: \(model.transport?.selectedDestination?.name ?? "nenhuma")"
                )
                if let match = model.selectedTransportMatch {
                    Label(
                        "\(match.variant.displayName) localizado como candidato de transporte",
                        systemImage: "cable.connector"
                    )
                    .foregroundStyle(LabTheme.draft)
                    Text("A identidade só será confirmada pela consulta explícita da próxima etapa.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                } else if model.selectedSourceMatchesProfile && model.selectedDestinationMatchesProfile {
                    Label(
                        "Entrada e saída pertencem a variantes diferentes; escolha um par da mesma variante.",
