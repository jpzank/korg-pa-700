...
struct ShowRepertoireWorkspace: View {
    ...
    private func importSelectedPDFs(_ result: Result<[URL], Error>) {
        ...
        Task { @MainActor in
            ...
            var failures: [String] = []
            for url in urls {
                do { extracted.append(try await ShowPDFImporter.extractPreset(from: url)) }
...
struct ShowMusicEditorPanel: View {
    ...
    @Binding var backdropCue: BackdropCue
    ...
    private var backdropSection: some View {
        GroupBox("Backdrop") {
            VStack(alignment: .leading, spacing: 13) {
                Toggle("Usar visual nesta música", isOn: $backdropCue.isEnabled)

                LabeledContent("Paleta") {
                    Picker("Paleta do backdrop", selection: $backdropCue.palette) {
                        ForEach(BackdropPalette.allCases) { palette in
                ...
                HStack(spacing: 7) {
                    ForEach(Array(backdropSwatches.enumerated()), id: \.offset) { _, color in
                        Circle()
                            .fill(color)
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(backdropCue.palette.displayName)
                            .font(.callout.weight(.semibold))
                        Text(backdropCue.palette.summary)
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                }
                ...
                LabeledContent("Ambiente") {
                    Picker("Movimento ambiente", selection: $backdropCue.ambientMode) {
                        ForEach(BackdropAmbientMode.allCases) { mode in
                ...
                backdropSlider(
                    title: "Intensidade",
                    value: $backdropCue.intensity,
                    help: "Controla o brilho máximo das pinceladas."
                )
                backdropSlider(
                    title: "Persistência",
                    value: $backdropCue.persistence,
                    help: "Controla por quanto tempo a tinta permanece antes de desaparecer."
                )
                backdropSlider(
                    title: "Sensibilidade",
                    value: $backdropCue.sensitivity,
                    help: "Controla quanto a velocity aumenta o tamanho das pinceladas."
                )

                Label(
                    "O visual só muda ao aplicar a música ou usar Ativar visual no Show.",
    ...
    private func backdropSlider(title: String, value: Binding<Double>, help: String) -> some View {
    ...
    private var backdropSwatches: [Color] {
        switch backdropCue.palette {
        case .forestLight:
            return [
                Color(red: 0.08, green: 0.83, blue: 0.69),
                Color(red: 0.18, green: 0.62, blue: 0.95),
                Color(red: 0.96, green: 0.58, blue: 0.20)
            ]
        case .deepLagoon:
            return [
                Color(red: 0.06, green: 0.74, blue: 0.92),
                Color(red: 0.14, green: 0.42, blue: 0.94),
                Color(red: 0.18, green: 0.88, blue: 0.72)
            ]
        case .ember:
            return [
                Color(red: 0.98, green: 0.30, blue: 0.20),
                Color(red: 0.98, green: 0.62, blue: 0.18),
                Color(red: 0.88, green: 0.16, blue: 0.48)
            ]
        case .orchid:
            return [
                Color(red: 0.76, green: 0.26, blue: 0.94),
                Color(red: 0.96, green: 0.24, blue: 0.62),
                Color(red: 0.28, green: 0.46, blue: 0.98)
            ]
        case .paper:
            return [
                Color(red: 0.06, green: 0.48, blue: 0.42),
                Color(red: 0.78, green: 0.20, blue: 0.34),
                Color(red: 0.90, green: 0.48, blue: 0.08)
            ]
        }
    }
