...
private struct ArrangerLabCommands: Commands {
    ...
    var body: some Commands {
        CommandMenu("Arranger Lab") {
            ...
            Button("Abrir Backdrop") {
                model.presentBackdrop(.preview)
                openWindow(id: "backdrop")
            }
                .keyboardShortcut("4", modifiers: [.command])
            Button("Blackout do Backdrop") { model.blackoutBackdrop() }
        ...
        VStack(spacing: 0) {
            ...
            NavigationSplitView {
                ...
                Group {
                    ...
                    case .korgMedia: KORGMediaInspectorView()
...
struct ConnectionView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            Form {
                Picker("Entrada", selection: $model.selectedSourceID) {
                    Text("Nenhuma").tag(Int32?.none)
                    ForEach(model.sources) { endpoint in
                        Text(
                            [endpoint.name, endpoint.manufacturer, endpoint.model, String(endpoint.id)]
                                .compactMap { $0 }
                                .joined(separator: " · ")
                        )
                ...
                Picker("Saída", selection: $model.selectedDestinationID) {
                    Text("Nenhuma").tag(Int32?.none)
                ...
                HStack {
                    Button("Conectar") { model.connect() }.buttonStyle(.borderedProminent)
                    Button("Localizar instrumento") { model.autoConnect() }
                    Button("Desconectar") { model.disconnect() }.disabled(!model.connected)
                }
            }.formStyle(.grouped)
            GroupBox("Candidato de transporte") {
                VStack(alignment: .leading, spacing: 7) {
                    if let match = model.selectedTransportMatch {
                        Label(
                            "\(match.variant.displayName) · nomes dos dois endpoints reconhecidos",
                            systemImage: "cable.connector"
                        )
                        .foregroundStyle(LabTheme.draft)
                        Text("Entrada: \(match.source.name)")
                        Text("Saída: \(match.destination.name)")
                        if match.evidence.contains(.manufacturer) {
                            Text("Fabricante CoreMIDI também coincide com o perfil.")
                                .foregroundStyle(.secondary)
                        }
                        if match.evidence.contains(.model) {
                            Text("Modelo CoreMIDI também coincide com a variante.")
                                .foregroundStyle(.secondary)
                        }
                        Text("Este é um candidato. Use a identidade universal abaixo para confirmar o instrumento.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    } else {
                        Text("Nenhum par reconhecido está selecionado.")
                            .foregroundStyle(.secondary)
                    }
                }
                .frame(maxWidth: .infinity, alignment: .leading)
            ...
            GroupBox("Preset MIDI ArrangerLab: configuração física") {
                VStack(alignment: .leading, spacing: 12) {
                    Text("Crie no primeiro slot vazio; nunca sobrescreva um preset.")
                        .fontWeight(.semibold)
                    Text("Ch 1 Upper1 · Ch 2 Upper2 · Ch 3 Upper3 · Ch 4 Lower · Ch 16 Control")
                    Text("Libere CC, PC e apenas SysEx conhecidos. Firmware permanece em 1.5.0.")
                        .foregroundStyle(.secondary)
                    Divider()
                    PA700MIDIPresetReferenceView()
                }
                .frame(maxWidth: .infinity, alignment: .leading)
                .padding(8)
            }
