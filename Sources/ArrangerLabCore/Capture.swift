...
public enum ArrLabPackage {
    ...
    /// Publishes a complete package without exposing a partially-written destination.
    ///
    /// Audio sources are keyed by the IDs in `experiment.analysis.audioEvidence`.
    /// Every referenced recording must use a safe path below `audio/` and have a
    /// corresponding regular source file. If `requestedURL` already exists, a
    /// numbered sibling is selected and returned instead of overwriting it.
    @discardableResult
    public static func publish(
        _ experiment: ArrLabExperiment,
        to requestedURL: URL,
        audioAttachments: [UUID: URL] = [:],
        bpm: Double = 120,
        ppqn: UInt16 = 480
    ) throws -> URL {
        guard requestedURL.isFileURL, !requestedURL.lastPathComponent.isEmpty else {
            throw ArrangerLabError.invalidValue("package destination must be a file URL")
        }
        guard bpm.isFinite, bpm > 0, ppqn > 0 else {
            throw ArrangerLabError.invalidValue("package export tempo and PPQN must be positive")
        }

        let fm = FileManager.default
        let parentURL = requestedURL.deletingLastPathComponent()
        try fm.createDirectory(at: parentURL, withIntermediateDirectories: true)
        let initialDestination = uniqueDestination(for: requestedURL, fileManager: fm)
        let stagingURL = uniqueStagingURL(siblingOf: initialDestination, fileManager: fm)
        ...
        try save(experiment, to: stagingURL)
        try copyAudioAttachments(
            experiment.analysis.audioEvidence,
            sourcesByID: audioAttachments,
            to: stagingURL,
            fileManager: fm
        )
        try CaptureExporter.writeCSV(
            events: experiment.events,
            to: stagingURL.appendingPathComponent("export.csv"),
            bpm: bpm,
            ppqn: Int(ppqn)
        )
        try CaptureExporter.smf(events: experiment.events, bpm: bpm, ppqn: ppqn)
            .write(to: stagingURL.appendingPathComponent("export.mid"), options: .atomic)

        try validatePublishedPackage(
            at: stagingURL,
            expectedExperiment: experiment
        )
        ...
        while true {
            do {
                ...
                guard fm.fileExists(atPath: destinationURL.path) else { throw error }
                destinationURL = uniqueDestination(for: requestedURL, fileManager: fm)
            }
        }
    }
    ...
    /// Loads only a complete package produced by `publish`, rejecting partial
    /// packages and any symlinked or escaped evidence path.
    public static func loadPublished(from url: URL) throws -> ArrLabExperiment {
        do {
            ...
            guard url.isFileURL,
                ...
                throw ArrangerLabError.corruptCapture("package root is missing or unsafe")
            }

            for name in ["manifest.json", "analysis.json", "events.jsonl", "export.csv", "export.mid"] {
                let values = try url.appendingPathComponent(name).resourceValues(
                    forKeys: [.isRegularFileKey, .isSymbolicLinkKey]
                )
                guard values.isRegularFile == true,
                      values.isSymbolicLink != true else {
                    throw ArrangerLabError.corruptCapture(
                        "package file is missing or unsafe: \(name)"
                    )
                }
            }

            let audioDirectoryValues = try url.appendingPathComponent("audio").resourceValues(
                forKeys: [.isDirectoryKey, .isSymbolicLinkKey]
            )
            guard audioDirectoryValues.isDirectory == true,
                  audioDirectoryValues.isSymbolicLink != true else {
                throw ArrangerLabError.corruptCapture(
                    "package audio directory is missing or unsafe"
                )
            }

            let loaded = try load(from: url)
            try validateAudioReferences(loaded.analysis.audioEvidence, in: url)

            let csvPrefix = try filePrefix(
                at: url.appendingPathComponent("export.csv"),
                count: 24
            )
            guard csvPrefix.starts(with: Data("tick,status,data1,data2\n".utf8)) else {
                throw ArrangerLabError.corruptCapture("CSV export is invalid")
            }
            let smfPrefix = try filePrefix(
                at: url.appendingPathComponent("export.mid"),
                count: 4
            )
            guard smfPrefix == Data("MThd".utf8) else {
                throw ArrangerLabError.corruptCapture("SMF export is invalid")
            }
            return loaded
    ...
    private static func copyAudioAttachments(
        _ evidenceRecords: [AudioEvidenceRecord],
        sourcesByID: [UUID: URL],
        to packageURL: URL,
        fileManager fm: FileManager
    ) throws {
        guard Set(evidenceRecords.map(\.id)).count == evidenceRecords.count else {
            throw ArrangerLabError.corruptCapture("audio evidence IDs must be unique")
        }
        guard Set(evidenceRecords.map(\.relativePath)).count == evidenceRecords.count else {
            throw ArrangerLabError.corruptCapture("audio evidence paths must be unique")
        }

        for evidence in evidenceRecords {
            let destinationURL = try audioDestination(
                for: evidence.relativePath,
                in: packageURL
            )
            guard let sourceURL = sourcesByID[evidence.id] else {
                throw ArrangerLabError.corruptCapture(
                    "missing audio attachment for \(evidence.relativePath)"
                )
            }
            guard sourceURL.isFileURL else {
                throw ArrangerLabError.invalidValue(
                    "audio attachment source must be a file URL"
                )
            }
            ...
            guard sourceValues.isRegularFile == true,
                ...
                throw ArrangerLabError.invalidValue(
                    "audio attachment must be a regular file: \(evidence.relativePath)"
                )
            }
            ...
            guard destinationValues.isRegularFile == true,
                  ...
                  destinationValues.fileSize == sourceValues.fileSize else {
                throw ArrangerLabError.corruptCapture(
                    "audio attachment copy could not be validated: \(evidence.relativePath)"
                )
            }
        }
    }

    private static func validatePublishedPackage(
        at packageURL: URL,
        expectedExperiment: ArrLabExperiment
    ) throws {
        let loaded = try loadPublished(from: packageURL)
        guard loaded.manifest.experimentID == expectedExperiment.manifest.experimentID,
              loaded.events == expectedExperiment.events,
              loaded.analysis.audioEvidence == expectedExperiment.analysis.audioEvidence else {
            throw ArrangerLabError.corruptCapture("package validation did not match source evidence")
        }
    }

    private static func validateAudioReferences(
        _ evidenceRecords: [AudioEvidenceRecord],
        in packageURL: URL
    ) throws {
        guard Set(evidenceRecords.map(\.id)).count == evidenceRecords.count,
              Set(evidenceRecords.map(\.relativePath)).count == evidenceRecords.count else {
            throw ArrangerLabError.corruptCapture(
                "audio evidence references must be unique"
            )
        }
        ...
        for evidence in evidenceRecords {
            let audioURL = try audioDestination(
                for: evidence.relativePath,
                in: packageURL
            )
            let resolvedAudio = audioURL.resolvingSymlinksInPath().standardizedFileURL
            guard resolvedAudio.path.hasPrefix(rootPrefix) else {
                throw ArrangerLabError.corruptCapture(
                    "referenced audio escapes the package: \(evidence.relativePath)"
                )
            }

            let components = evidence.relativePath.split(separator: "/").map(String.init)
            var currentURL = packageURL
            for (index, component) in components.enumerated() {
                ...
                guard values.isSymbolicLink != true else {
                    throw ArrangerLabError.corruptCapture(
                        "referenced audio path contains a symlink: \(evidence.relativePath)"
                    )
                }
                if index == components.indices.last {
                    guard values.isRegularFile == true else {
                        throw ArrangerLabError.corruptCapture(
                            "referenced audio is missing: \(evidence.relativePath)"
                        )
                    }
                } else {
                    guard values.isDirectory == true else {
                        throw ArrangerLabError.corruptCapture(
                            "referenced audio parent is invalid: \(evidence.relativePath)"
                        )
                    }
                }
            }
    ...
    private static func audioDestination(
        ...
        guard !relativePath.isEmpty,
            ...
            throw ArrangerLabError.invalidValue(
                "unsafe audio attachment path: \(relativePath)"
            )
        }
        ...
        return components.reduce(packageURL) { partialURL, component in
            partialURL.appendingPathComponent(component)
        }
    }

    private static func filePrefix(at url: URL, count: Int) throws -> Data {
    ...
    private static func uniqueDestination(
        ...
        guard fm.fileExists(atPath: requestedURL.path) else { return requestedURL }
        ...
        var suffix = 2
        while true {
            var candidate = parentURL.appendingPathComponent(
                "\(baseName)-\(suffix)",
                isDirectory: true
            )
    ...
    private static func uniqueStagingURL(
...
public enum CaptureExporter {
    public static func csv(events: [MIDIEvent], bpm: Double = 120, ppqn: Int = 480) -> String {
        let events = chronological(events)
    ...
    private static func chronological(_ events: [MIDIEvent]) -> [MIDIEvent] {
