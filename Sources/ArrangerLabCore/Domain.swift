...
public struct MIDIEvent: Codable, Identifiable, Equatable, Sendable {
    ...
    public let transportSessionID: UUID?
    ...
    public init(
        ...
        endpointName: String,
        protocolKind: MIDIProtocolKind = .midi1,
        rawBytes: [UInt8],
        message: MIDIMessage?,
        transportSessionID: UUID? = nil
    ) {
        self.id = id
...
public enum ArrangerLabError: LocalizedError, Equatable {
    ...
    case panicDeliveryFailed(failureCount: Int, firstError: String)
    ...
    public var errorDescription: String? {
        ...
        case .deviceIdentityRequired: return "The current MIDI connection has not confirmed the device identity"
        case let .panicDeliveryFailed(failureCount, firstError):
            return "Panic delivery failed for \(failureCount) messages: \(firstError)"
