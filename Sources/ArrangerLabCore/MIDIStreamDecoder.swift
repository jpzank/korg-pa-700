...
public enum MIDIStreamDecoderIssue: LocalizedError, Equatable, Sendable {
    case systemExclusiveTooLarge(limit: Int)

    public var errorDescription: String? {
        switch self {
        case let .systemExclusiveTooLarge(limit):
            return "Incoming SysEx exceeded the \(limit)-byte safety limit and was dropped"
        }
    }
}
...
public final class MIDIStreamDecoder {
    ...
    public init(maximumSysExBytes: Int = 1_048_576) {
        precondition(maximumSysExBytes >= 2, "SysEx limit must hold at least F0 and F7")
        self.maximumSysExBytes = maximumSysExBytes
    }
    ...
    public func feed(
        _ bytes: [UInt8],
        onIssue: ((MIDIStreamDecoderIssue) -> Void)? = nil
    ) -> [DecodedMIDIMessage] {
        ...
        for byte in bytes {
            ...
            if sysEx != nil {
                guard sysEx!.count < maximumSysExBytes else {
                    sysEx = nil
                    pendingStatus = nil
                    pendingData.removeAll(keepingCapacity: true)
                    pendingRaw.removeAll(keepingCapacity: true)
                    expectedData = 0
                    onIssue?(.systemExclusiveTooLarge(limit: maximumSysExBytes))
                    continue
                }
                sysEx!.append(byte)
                if byte == 0xF7 {
