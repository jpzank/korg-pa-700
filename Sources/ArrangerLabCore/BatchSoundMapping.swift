...
public struct BatchSoundCollector: Sendable {
    ...
    private var entryIndexBySelection: [MIDIProgramSelection: Int] = [:]
    ...
    public mutating func consume(
        ...
        case let .programChange(channel, program) where channel == requiredChannel:
            ...
            if let index = entryIndex(for: selection, expectedID: id) {
            ...
            indexEntryIfNeeded(entry, at: catalog.entries.index(before: catalog.entries.endIndex))
            appendToActiveScreen(entryID: id)
            return entry
        default:
            return nil
    ...
    public mutating func undoLastScreenCapture(now: Date = Date()) -> BatchSoundEntry? {
        guard let screenIndex = catalog.screens.lastIndex(where: \.isOpen),
              ...
              let entryIndex = entryIndexByID[entryID] else { return nil }
    ...
    public mutating func assignNames(
        ...
        for (entryID, name) in zip(screen.entryIDs, cleaned) {
            guard let index = entryIndexByID[entryID] else { continue }
    ...
    private func entryIndex(for selection: MIDIProgramSelection, expectedID: String) -> Int? {
        if let index = entryIndexBySelection[selection],
           catalog.entries[index].id == expectedID {
            return index
        }
        return entryIndexByID[expectedID]
    }

    private mutating func indexEntryIfNeeded(_ entry: BatchSoundEntry, at index: Int) {
        if entryIndexByID[entry.id] == nil {
            entryIndexByID[entry.id] = index
        }
        if entryIndexBySelection[entry.selection] == nil {
            entryIndexBySelection[entry.selection] = index
        }
    }

    private mutating func rebuildEntryIndexes() {
        entryIndexByID.removeAll(keepingCapacity: true)
        entryIndexBySelection.removeAll(keepingCapacity: true)
        entryIndexByID.reserveCapacity(catalog.entries.count)
        entryIndexBySelection.reserveCapacity(catalog.entries.count)
        for (index, entry) in catalog.entries.enumerated() {
            indexEntryIfNeeded(entry, at: index)
        }
    }
}
