...
func run() throws {
    ...
    try check(profile.schemaVersion == 2, "bundled profile uses transport-aware schema v2")
    guard let transportIdentity = profile.transportIdentity else {
        throw HarnessFailure(description: "bundled profile transport identity missing")
    }
    try check(
        transportIdentity.vendorID == 0x0944 &&
            transportIdentity.variants.map(\.productID) == [0x01C9, 0x01CB],
        "PA700 USB identity includes Standard and Oriental"
    )
    let standardTransportMatches = TransportMatcher.matches(
        identity: transportIdentity,
        sources: [MIDIEndpoint(id: 1, name: "Pa700 _ KEYBOARD", ref: 0)],
        destinations: [MIDIEndpoint(id: 2, name: "Pa700 SOUND", ref: 0)]
    )
    try check(
        standardTransportMatches.count == 1 && standardTransportMatches[0].variant.id == "standard",
        "transport matcher normalizes Standard endpoint names"
    )
    let orientalTransportMatches = TransportMatcher.matches(
        identity: transportIdentity,
        sources: [MIDIEndpoint(id: 3, name: "Pa700 Oriental KEYBOARD", ref: 0)],
        destinations: [MIDIEndpoint(id: 4, name: "Pa700 Oriental _ SOUND", ref: 0)]
    )
    try check(
        orientalTransportMatches.count == 1 && orientalTransportMatches[0].variant.id == "oriental",
        "transport matcher keeps Oriental endpoints in one variant"
    )
    let legacyProfile = InstrumentProfile(
        schemaVersion: 1,
        id: "legacy",
        manufacturer: "Test",
        model: "Legacy",
    ...
    let decodedLegacyProfile = try JSONDecoder().decode(
        InstrumentProfile.self,
        from: JSONEncoder().encode(legacyProfile)
    )
    try decodedLegacyProfile.validate()
    try check(
        decodedLegacyProfile.transportIdentity == nil,
        "profile schema v1 loads without transport identity"
    )
    try check(
        KORGMediaResourceKind(fileExtension: ".sTy") == .sty
            && KORGMediaResourceKind(fileExtension: "txt") == .unknown,
        "KORG media extensions classify case-insensitively"
    )
    let mediaInventory = KORGMediaInventory(
        sourceName: "Harness.SET",
        containerKind: .set,
        items: [
            .init(
                relativePath: "STYLE/Z.STY",
                fileExtension: "STY",
                resourceKind: .sty,
                sizeBytes: 2,
                functionalDirectory: "STYLE",
                status: .opaque
            ),
            .init(
                relativePath: "GLOBAL/A.MID",
                fileExtension: "MID",
                resourceKind: .mid,
                sizeBytes: 1,
                functionalDirectory: "GLOBAL",
                status: .recognized
            )
        ]
    )
    try check(
        mediaInventory.items.map(\.relativePath) == ["GLOBAL/A.MID", "STYLE/Z.STY"]
            && mediaInventory.summary.itemCount == 2
            && mediaInventory.summary.totalBytes == 3,
        "KORG media inventory sorts relative paths and summarizes metadata"
    )
    let mediaJSON = try mediaInventory.jsonData()
    let repeatedMediaJSON = try mediaInventory.jsonData()
    try check(
        mediaJSON == repeatedMediaJSON,
        "KORG media JSON export is deterministic"
    )
    try check(
        String(data: mediaJSON, encoding: .utf8)?.contains("/Users/") == false,
        "KORG media JSON does not expose an absolute user path"
    )
    ...
    try check(Set(styleCatalog.styles.map(\.address)).count == styleCatalog.styles.count, "factory Style MIDI addresses are unique")
    ...
    try check(ArrangerStyle.userBankNames.count == 12
        && ArrangerStyle.userBankNames[6] == "User 7"
        && ArrangerStyle.userBankNames[11] == "User 12", "PA700 User Style hierarchy exposes neutral labels for all 12 banks")
    ...
    let exampleCatalog = BundledShowCatalog(
        schemaVersion: 1,
        catalogID: "community-example",
        name: "Community Example",
        sourceFileName: "community-example.pdf",
        sourceURL: nil,
        entries: [
            BundledShowCatalogEntry(
                catalogSongID: "opening-song",
                presetID: UUID(uuidString: "10000000-0000-4000-8000-000000000001")!,
                ...
                endPage: 0,
                chartLines: [
                    ShowChartLine(kind: .section, text: "Intro"),
                    ShowChartLine(kind: .chords, text: "C F G"),
                    ShowChartLine(kind: .lyrics, text: "An original example line")
                ]
            ),
            BundledShowCatalogEntry(
                catalogSongID: "closing-song",
                presetID: UUID(uuidString: "10000000-0000-4000-8000-000000000002")!,
                songTitle: "Closing Song",
                artist: nil,
                originalKey: "G",
                startPage: 1,
                endPage: 1,
                chartLines: [
                    ShowChartLine(kind: .section, text: "Verse"),
                    ShowChartLine(kind: .chords, text: "G D Em C"),
                    ShowChartLine(kind: .lyrics, text: "A second original example line")
                ]
            )
        ]
    )
    let firstCatalogMerge = exampleCatalog.merging(presets: [], setLists: [], now: sceneDate)
    try check(
        firstCatalogMerge.presets.count == 2
            && firstCatalogMerge.importedCount == 2
            && firstCatalogMerge.setLists.count == 1
            && firstCatalogMerge.setLists[0].items.count == 2,
        "generic catalog import creates drafts and a complete set list"
    )
    try check(firstCatalogMerge.presets.allSatisfy { !$0.isConfirmed && $0.songBookNumber == nil }, "generic catalog import keeps songs blocked as drafts")
    let repeatedCatalogMerge = exampleCatalog.merging(
    ...
    try check(
        ...
        "generic catalog reimport is idempotent"
    )
    var editedCatalogPresets = firstCatalogMerge.presets
    editedCatalogPresets[0].songBookNumber = 123
    editedCatalogPresets[0].notes = "Local performance note"
    editedCatalogPresets[0].chartLines[0].text = "Local edit"
    let preservedCatalogMerge = exampleCatalog.merging(
        presets: editedCatalogPresets,
        setLists: firstCatalogMerge.setLists,
        now: sceneDate.addingTimeInterval(120)
    )
    try check(preservedCatalogMerge.presets[0] == editedCatalogPresets[0], "generic catalog reimport preserves local edits")
    var incompleteCatalogSetLists = firstCatalogMerge.setLists
    incompleteCatalogSetLists[0].items.removeFirst()
    let restoredCatalogSetListMerge = exampleCatalog.merging(
        presets: firstCatalogMerge.presets,
        setLists: incompleteCatalogSetLists,
        now: sceneDate.addingTimeInterval(180)
    )
    try check(restoredCatalogSetListMerge.setLists[0].items.count == 2, "generic catalog reimport restores a missing set-list song")
    ...
    let backdropCue = BackdropCue(
        presetID: showPreset.id,
        palette: .forestLight,
        intensity: 0.7,
        persistence: 0.65,
        sensitivity: 0.6,
    ...
    let backdropURL = FileManager.default.temporaryDirectory
        .appendingPathComponent("\(UUID().uuidString)-backdrop-cues.json")
    defer { try? FileManager.default.removeItem(at: backdropURL) }
    try BackdropCueStore.save([backdropCue], to: backdropURL)
    let loadedBackdropCues = try BackdropCueStore.load(from: backdropURL)
    try check(
        loadedBackdropCues == [backdropCue],
        "backdrop cues persist separately from PA700 show presets"
    )
    var backdropReducer = BackdropEventReducer(maximumStrokes: 16)
    backdropReducer.activate(backdropCue, at: 1)
    _ = backdropReducer.consume(inputEvent(.noteOn(channel: 0, note: 60, velocity: 80), at: 10))
    _ = backdropReducer.consume(inputEvent(.noteOn(channel: 0, note: 64, velocity: 90), at: 30_000_000))
    _ = backdropReducer.consume(inputEvent(.noteOn(channel: 0, note: 67, velocity: 100), at: 60_000_000))
    try check(
        backdropReducer.state.strokes.contains(where: { $0.kind == .chordBloom }),
        "backdrop reducer turns clustered PA700 notes into a chord bloom"
    )
    _ = backdropReducer.consume(inputEvent(.controlChange(channel: 0, controller: 64, value: 127), at: 70_000_000))
    _ = backdropReducer.consume(inputEvent(.noteOff(channel: 0, note: 60, velocity: 0), at: 80_000_000))
    let sustainedStroke = backdropReducer.state.strokes.first(where: { $0.note == 60 && $0.kind == .note })
    try check(
        sustainedStroke?.releasedAtNanoseconds == nil,
        "backdrop sustain keeps released notes visible until pedal up"
    )
    _ = backdropReducer.consume(inputEvent(.controlChange(channel: 0, controller: 64, value: 0), at: 90_000_000))
    try check(
        backdropReducer.state.strokes.first(where: { $0.id == sustainedStroke?.id })?.releasedAtNanoseconds == 90_000_000,
        "backdrop pedal up releases sustained paint"
    )
    backdropReducer.blackout(at: 100_000_000)
    try check(
        backdropReducer.state.isBlackout && backdropReducer.state.strokes.isEmpty,
        "backdrop blackout clears performance paint without changing PA700 state"
    )
    ...
    if CommandLine.arguments.contains("--skip-coremidi") {
        print("SKIP  CoreMIDI integration checks (--skip-coremidi)")
    } else {
    ...
    try check(MIDISourceCreate(client, "Arranger Lab Virtual Source" as CFString, &virtualSource) == noErr, "CoreMIDI virtual source")
    defer { MIDIEndpointDispose(virtualSource) }
    var sourceUniqueID: Int32 = 0
    try check(MIDIObjectGetIntegerProperty(virtualSource, kMIDIPropertyUniqueID, &sourceUniqueID) == noErr, "virtual source Unique ID")
    ...
    let protectedTransport = try MIDITransport(requiresSessionAuthorization: true)
    protectedTransport.refreshEndpoints()
    try protectedTransport.connect(sourceID: sourceUniqueID, destinationID: uniqueID)
    do {
        try protectedTransport.send(.noteOn(channel: 0, note: 60, velocity: 80))
        throw HarnessFailure(description: "unconfirmed MIDI session accepted a device mutation")
    } catch ArrangerLabError.deviceIdentityRequired {
        passed += 1
        print("PASS  device mutation requires current-session identity")
    }
    try protectedTransport.authorizeCurrentConnection()
    try protectedTransport.send(.noteOn(channel: 0, note: 60, velocity: 80))

    let inputSemaphore = DispatchSemaphore(value: 0)
    var receivedLargeSysEx = false
    var priorSessionInputEvent: MIDIEvent?
    protectedTransport.onEvent = { event in
        if event.direction == .input,
           case let .systemExclusive(bytes) = event.message,
           bytes.count == 300 {
            receivedLargeSysEx = true
            priorSessionInputEvent = event
            inputSemaphore.signal()
        }
    }
    let packetStorage = UnsafeMutableRawPointer.allocate(byteCount: 1_024, alignment: 4)
    defer { packetStorage.deallocate() }
    let packetList = packetStorage.bindMemory(to: MIDIPacketList.self, capacity: 1)
    let firstPacket = MIDIPacketListInit(packetList)
    let largeSysEx = [UInt8(0xF0)] + Array(repeating: UInt8(0x01), count: 298) + [0xF7]
    _ = largeSysEx.withUnsafeBufferPointer { bytes in
        MIDIPacketListAdd(packetList, 1_024, firstPacket, 0, bytes.count, bytes.baseAddress!)
    }
    try check(MIDIReceived(virtualSource, packetList) == noErr, "dynamic CoreMIDI packet delivered")
    try check(inputSemaphore.wait(timeout: .now() + 2) == .success && receivedLargeSysEx, "CoreMIDI input preserves packets larger than 256 bytes")

    try protectedTransport.connect(sourceID: sourceUniqueID, destinationID: uniqueID)
    if let priorSessionInputEvent {
        do {
            try protectedTransport.authorizeCurrentConnection(from: priorSessionInputEvent)
            throw HarnessFailure(description: "stale queued input authorized a replacement MIDI session")
        } catch ArrangerLabError.deviceIdentityRequired {
            passed += 1
            print("PASS  stale queued input cannot authorize a replacement session")
        }
    } else {
        throw HarnessFailure(description: "missing prior-session input event")
    }
    do {
        try protectedTransport.send(.noteOn(channel: 0, note: 60, velocity: 80))
        throw HarnessFailure(description: "reconnected MIDI session retained stale authorization")
    } catch ArrangerLabError.deviceIdentityRequired {
        passed += 1
        print("PASS  reconnect invalidates device identity authorization")
    }
    protectedTransport.close(sendPanic: false)
